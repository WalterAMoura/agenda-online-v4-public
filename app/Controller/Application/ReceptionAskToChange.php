...
class ReceptionAskToChange extends Page
{
    /**
     * @var array
     */
    ...
    private static function getWho2(int $selected = null, string $schedulerDate): string
    {
        ...
        $results = EntityReceptionTeam::getReceptionTeam($where, $order);
        while ($obReceptionTeam = $results->fetchObject(EntityReceptionTeam::class)){
            $options .= View::render('application/modules/reception/forms/select2',[
                'optionValue' => $obReceptionTeam->id,
                'optionName' => (in_array($obReceptionTeam->id, $data))? $obReceptionTeam->complete_name . ' (Não permitido)' : $obReceptionTeam->complete_name,
                'selected' => ($obReceptionTeam->id === $selected)? 'selected' : null,
                'disabled' => (in_array($obReceptionTeam->id, $data))?'disabled' : null
            ]);
        }
    ...
    private static function getAskToChangeItems(int $userId, int $levelId, string $type): string
    {
        ...
        //$obReceptionTeam = self::getUserSoundTeam($userId);
        $obReceptionTeam = EntityReceptionTeam::getReceptionTeamByLinkedUserId($userId);

        if(!$obReceptionTeam instanceof EntityReceptionTeam && $levelId != -1) {
            return $itens;
        }

        $query = (!$obReceptionTeam instanceof EntityReceptionTeam) ? null : (($type == 'requested') ? 'current_linked_user_id = ' . $obReceptionTeam->id ?? 0 : 'new_linked_user_id = ' . $obReceptionTeam->id ?? 0);
        $orderBy = 'id ASC';
        $results = EntityReceptionAskToChange::getAskToChange($query, $orderBy);
        // renderiza itens
        while ($obAskToChange = $results->fetchObject(EntityReceptionAskToChange::class)){
            $schedulerDate = new DateTime($obAskToChange->scheduler_date);
            $schedulerDate = $schedulerDate->format('d-m-Y');

            $itens .= View::render('application/modules/reception/items/my-ask-to-change',[
                'id' => $obAskToChange->id,
    ...
    /**
     * @param int $userId
     * @return EntityReceptionTeam|integer
     */
    ...
    public static function getListMyAskToChange(Request $request, string $type)
    {
        ...
        $content = View::render('application/modules/reception/forms/my-ask-to-change',[
        ...
        return parent::getPanel( $obOrganization->full_name . ' | ' . ($type == 'requested')?'Minhas Solicitações de Troca': 'Solicitações de Troca Recebidas',$content,'reception');
    }
    ...
    private static function getSchedulerDate(string $schedulerDate)
    {
        ...
        $results = EntityReceptionTeamLineup::getReceptionTeamLineup($query);

        while ($obReceptionTeamLineup = $results->fetchObject(EntityReceptionTeamLineup::class)){
            $data[] = $obReceptionTeamLineup->reception_team_id;
        }
        //Debug::debug($obReceptionTeamLineup->fetchAll());
    ...
    public static function getAskToChange(Request $request, int $id)
    {
        ...
        if($obReceptionTeamLineup->linked_user_id !== $userId){
        ...
        $content = View::render('application/modules/reception/forms/ask-to-change', [
            'title' => 'Pedir Troca',
            'breadcrumbItem' => 'Pedir Troca',
            'schedulerDate' => $schedulerDate,
            'who' => self::getWho($obReceptionTeamLineup->reception_team_id),
            'who2' => self::getWho2($obReceptionTeamLineup->reception_team_id, $schedulerDate),
            'btnTipo' => 'success',
        ...
        // Retorna a pagina completa
        return parent::getPanel( $obOrganization->full_name . ' | Pedir Troca',$content,'reception');
    ...
    {
        // Obtém do banco de dados
        $obAskToChange = EntityReceptionAskToChange::getAskToChangeBySchedulerId($id);
...
//        }
        ...
        if($obAskToChange->status == 1){
            $request->getRouter()->redirect('/application/reception/ask-to-change/'.$obAskToChange->scheduler_id.'?status=ask_duplicated');
        }
        ...
        if(General::isNullOrEmpty($postVars['who']) ){
            $request->getRouter()->redirect('/application/reception/ask-to-change/'.$id.'?status=failed');
        }
        ...
        // Nova instancia
        $obAskToChange = new EntityReceptionAskToChange();
        $obAskToChange->current_linked_user_id = $obReceptionTeamLineup->reception_team_id;
        ...
        // Redireciona
        $request->getRouter()->redirect('/application/reception/ask-to-change/'.$obAskToChange->scheduler_id.'?status=ask_created');
    ...
    public static function setAskToChangeStatus(Request $request, int $id, int $status): array
    {
        ...
        // Obtém do banco de dados
        $obAskToChange = EntityReceptionAskToChange::getAskToChangeById($id);

        if($status === 4) {
            // Obtém do banco de dados
            $obReceptionTeamLineup = EntityReceptionTeamLineup::getReceptionTeamLineupById($obAskToChange->scheduler_id);
            ...
            $obReceptionTeam = EntityReceptionTeam::getReceptionTeamById($obAskToChange->new_linked_user_id);

            if($obReceptionTeam->linked_user_id !== $userId){
                throw new Exception(Error::getError($request,403));
            }
        }
        ...
        if($obAskToChange->status != 1){
            $request->getRouter()->redirect('/application/reception/ask-to-change/received?status=ask_duplicated');
        }
        ...
        // Redireciona
        $request->getRouter()->redirect('/application/reception/ask-to-change/received?status=updated');
    ...
    private static function setUpdateScheduler(int $idAskToChange)
    {
        ...
        $obAskToChange = EntityReceptionAskToChange::getAskToChangeById($idAskToChange);
        ...
        $obReceptionTeamLineup->id = $schedulerId;
        $obReceptionTeamLineup->reception_team_id = $newLinkedUserId;
        $obReceptionTeamLineup->updateNewLinkedUser();
