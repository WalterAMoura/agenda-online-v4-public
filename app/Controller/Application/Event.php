...
class Event extends Page
{
    ...
    private static array $modules = [
        'loadTable' => [
            'script' => 'application/js/scripts/table-events'
        ]
    ];
    ...
    private static function getEventItems($request, &$obPagination)
    {
        ...
        $results = EntityEvent::getEvents(null);

        //Renderiza o item
        while ($obEvent = $results->fetchObject(EntityEvent::class)){
            $itens .= View::render('application/modules/event/item',[
                'id' => $obEvent->id,
                'data' => $obEvent->start,
    ...
    private static function getAllowManagerEvent()
    {
        ...
        $obAccessModules = EntityAccessModules::getAccessModuleByIdByLevelId($levelId,46);
    ...
    public static function getEvents($request)
    {
        ...
        $content = View::render('application/modules/event/index',[
        ...
        return parent::getPanel( $obOrganization->full_name . ' | Agenda',$content,'event',self::getScriptEvent($request));
    }
    ...
    private static function getStatusEvents(Request $request, string $selected = null)
    {
        ...
        while ($obStatusEvent = $results->fetchObject(EntityEventStatus::class)){
            $options .= View::render('application/modules/event/select',[
    ...
    {
        //$modal = '';
        return View::render('application/modules/event/modal-view',[
            'title' => $title,
    ...
    {
        //$modal = '';
        return View::render('application/modules/event/modal',[
            'title' => $title,
    ...
    public static function getNewEvent($request)
    {
        ...
        $content = View::render('application/modules/event/calendar',[
        ...
        // Retorna a pagina completa
        return parent::getPanel($obOrganization->full_name . ' | Agenda',$content,'event');
    }
    ...
    public static function setNewEvent(Request $request): void
    {
        ...
        $primeiroHino = $postVars['primeiroHino'] ?? '';
        $hinoFinal = $postVars['hinoFinal'] ?? '';
        $tema = $postVars['tema'] ?? '';
        ...
        $obEvent->hino_inicial = $primeiroHino;
        $obEvent->hino_final = $hinoFinal;
        $obEvent->status_id = $statusEvent;
        $obEvent->orador = $orador;
        ...
        // Redireciona
        $request->getRouter()->redirect('/application/event/home?status=created');
    }
    ...
    public static function setNewEditEvent(Request $request): void
    {
        ...
        // Redireciona
        $request->getRouter()->redirect('/application/event/home?status=updated');
    }
    ...
    public static function getDeleteEvent($request, $id)
    {
        ...
        //Conteúdo do formulário
        $content = View::render('application/modules/event/delete',[
    ...
    public static function setDeleteEvent($request, $id)
    {
        ...
        // Redireciona
        $request->getRouter()->redirect('/application/event/home?status=deleted');
