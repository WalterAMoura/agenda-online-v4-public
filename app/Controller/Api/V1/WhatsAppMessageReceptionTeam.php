...
class WhatsAppMessageReceptionTeam extends Api
{
    ...
    public static function sendMessage(Request $request): array
    {
        ...
        $results = EntityReceptionTeamLineup::getReceptionTeamLineup($where,null,null,$groupBY,$fields);
        ...
        while ($obReceptionTeamLineup = $results->fetchObject(EntityReceptionTeamLineup::class)) {
            ...
            $schedulerId = $obReceptionTeamLineup->id;
            $obInviteMessageWhatsApp = EntityAcceptedInviteReception::getSendMessageBySchedulerId($schedulerId);
            if($obInviteMessageWhatsApp instanceof EntityAcceptedInviteReception){
                ...
                $to = '55' . $obReceptionTeamLineup->contato;
                $bodyParams = [
                    [
                        "type" => "text",
                        "text" => $obReceptionTeamLineup->completed_name
                    ],
                    [
                        "type" => "text",
                        "text" => $schedulerDate
                    ...
                    [
                        "type" => "text",
                        "text" => ($obReceptionTeamLineup->day_short_description == 'Sab') ? "08:45" : (($obReceptionTeamLineup->day_short_description == 'Dom') ? "18:45" : "19:45")
                    ],
                    [
                        "type" => "text",
                        "text" => $obReceptionTeamLineup->day_long_description
                    ]
                ];
                ...
                $sendResponse = self::send($to, TEMPLATE_SEND_RECEPTION_WHATSAPP, $headerParams, $bodyParams, $buttonParams);
                ...
                if ($sendResponse['messages'][0]['message_status'] == 'accepted') {
                    ...
                    self::setSendMessageWhatsApp($obReceptionTeamLineup->reception_team_id, $to, $messageId, $messageStatus, json_encode($sendResponse));
                    self::setInviteMessageWhatsApp($obReceptionTeamLineup->id, $obReceptionTeamLineup->reception_team_id, $messageId);
                }
            }
        }
    ...
    public static function reminder(Request $request): array
    {
        ...
        $results = EntityAcceptedInviteReception::getAcceptedInvite($where,null,null,$groupBY,$fields);
        //Debug::debug($results->fetchAll());

        if($results->rowCount() === 0) throw new Exception("Nenhum registro encontrado.", 404);

        // Processa cada linha da escala
        while ($obAcceptedInvite = $results->fetchObject(EntityAcceptedInviteReception::class)) {
            ...
            if($sendResponse['messages'][0]['message_status'] == 'accepted'){
                ...
                self::setSendMessageWhatsApp($obAcceptedInvite->receptionteam_id,$to,$messageId,$messageStatus,json_encode($sendResponse));
                //self::setInviteMessageWhatsApp($obReceptionTeamLineup->id, $obReceptionTeamLineup->sound_team_id,$messageId);
            }
        }
    ...
    {
        $obSendMessageWhatsApp = new EntitySendMessageWhatsAppReception();
        $obSendMessageWhatsApp->receptionteam_id = $teamId;
    ...
    {
        $obInviteMessageWhatsApp = new EntityAcceptedInviteReception();
        $obInviteMessageWhatsApp->receptionteam_id = $teamId;
    ...
    private static function setInvitationAccepted($messageId, $messageStatus,$messageTimestamp, $recipientId, $messageText, Request $request)
    {
        ...
        $obSendMessageWhatsApp =  EntitySendMessageWhatsAppReception::getSendMessageByIdMessage($messageId);
        if(!$obSendMessageWhatsApp instanceof EntitySendMessageWhatsAppReception){
    ...
    private static function setUpdateMessageWebhook($messageId, $messageStatus,$messageTimestamp, $recipientId, Request $request): array
    {
        ...
        $receptionteamId = $obSendMessageWhatsApp->receptionteam_id;
        ...
        if($currentStatus === $messageStatus){
            ...
            $obSendMessageWhatsApp->receptionteam_id = $receptionteamId;
